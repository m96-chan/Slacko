...
func Render(text string, users map[string]slack.User, channels map[string]string, enabled bool, syntaxTheme string) string {
	if !enabled {
	...
	for _, seg := range segments {
		if seg.isCode {
			b.WriteString(renderCodeBlock(seg.lang, seg.code, syntaxTheme))
		} else {
			b.WriteString(renderInline(seg.text, users, channels))
		}
	}
...
func renderCodeBlock(lang, code string, syntaxTheme string) string {
	...
	if lang != "" {
		buf.WriteString("[gray::d]" + tview.Escape(lang) + "[-::-]")
	}
	buf.WriteString("\n")
...
func renderInline(text string, users map[string]slack.User, channels map[string]string) string {
	...
	text = slackTokenRe.ReplaceAllStringFunc(text, func(match string) string {
		...
		rendered := renderSlackToken(inner, users, channels)
...
func renderSlackToken(inner string, users map[string]slack.User, channels map[string]string) string {
	switch {
	// User mention: @U123 or @U123|name.
	case strings.HasPrefix(inner, "@"):
		return renderUserMention(inner[1:], users)

	// Channel mention: #C123 or #C123|name.
	case strings.HasPrefix(inner, "#"):
		return renderChannelMention(inner[1:], channels)

	// Special mentions: !here, !channel, !everyone.
	case strings.HasPrefix(inner, "!"):
		return renderSpecialMention(inner[1:])
...
func renderUserMention(token string, users map[string]slack.User) string {
	...
	return "[yellow::b]@" + tview.Escape(name) + "[-::-]"
}

// renderChannelMention renders #C123 or #C123|name.
func renderChannelMention(token string, channels map[string]string) string {
	parts := strings.SplitN(token, "|", 2)
	channelID := parts[0]
	...
	return "[cyan::b]#" + tview.Escape(name) + "[-::-]"
}

// renderSpecialMention renders !here, !channel, !everyone.
func renderSpecialMention(token string) string {
	parts := strings.SplitN(token, "|", 2)
	keyword := parts[0]
	...
	return "[yellow::bu]" + tview.Escape(label) + "[-::-]"
}

// renderLink renders a URL or URL|label.
func renderLink(token string) string {
	parts := strings.SplitN(token, "|", 2)
	url := parts[0]

	if len(parts) == 2 && parts[1] != "" {
		label := parts[1]
		return "[blue::u]" + tview.Escape(label) + "[-::-]"
	}

	return "[blue::u]" + tview.Escape(url) + "[-::-]"
}
